# if you have created an entrez_key (allows you to work faster on the ncbi server) enter it here: # Eva added
options(ENTREZ_KEY="d48b9acccd829282ff4386f716341ed2f608") # Eva added
...
file<-"./seqnames_Eukaryota_nobarcode.txt" ## File containing sequence names
...
get_taxinfo<-function(taxnames){
  ...
  for( i in 1:length(names)){
    col[[i]] = POST(url="https://www.ncbi.nlm.nih.gov/Taxonomy/TaxIdentifier/tax_identifier.cgi",
                    config=list(timeout(300000)),   # Eva added max timeout of 5 min
                    encode="form",
    ...
    Sys.sleep(5) # Eva added suspended execution of 5 s
  }
...
parentdetails$command<-paste("perl scripts/taxdump_edit/taxdump_edit.pl -names scripts/taxdump_edit/names.dmp -nodes scripts/taxdump_edit/nodes.dmp -taxa ", paste0("\'",parentdetails$oldname,"\'"),
